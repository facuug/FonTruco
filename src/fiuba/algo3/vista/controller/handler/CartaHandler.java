...
public class CartaHandler implements EventHandler<Event> {
	...
	public CartaHandler(List<ImageView> cartasDeMano, List<List<ImageView>> cartasJugando, Carta cartaQueRepresenta,
			Label puntosEquipoUno, Label puntosEquipoDos) {
	...
	private void habilitarCartas() {
			...
			for (ImageView carta : cartasAHabilitar) {
				if (!carta.isDisable()) {
					File fileDorso = new File(
							"src/fiuba/algo3/vista/recursos/carta/CARTA_JUMBO_BICYCLE_52_EN_1_DORSO_AZUL_-_DORSO.jpg");
	...
	public void setCartaQueSoy(Carta unaCarta) {
		cartaQueSoy = unaCarta;
	}

	public Carta getCartaQueSoy() {
		return cartaQueSoy;
	}
}
